...
function next_turn() {
  ...
  xp +=
    Math.floor(
      Math.max(0, Math.min(1 + Math.ceil(currentpop / 5), food - currentpop)) **
        0.5
    ) *
    10 *
  ...
  if (difficulty > 20) {
    for (const p of m.marketselections) {
      p.turnmodify();
      if (p.title != "Mysterious Artifact") {
...
function displayUI(turn = false) {
  ...
  if (m.phase > 1) {
    ...
    document.getElementById("boss_health_text").innerHTML =
      "boss: " +
      shorten(Math.max(0, m.bhealth)) +
      "/" +
      shorten(m.totalbhealth);
    ...
    document.getElementById("boss_health_text").innerHTML =
      "enemy military: " +
      shorten(Math.max(0, enemypower)) +
      "/" +
      shorten(totalenemypower);
  } else {
  ...
  while (xp >= totalxp) {
    if (difficulty > 5) {
      document.getElementById("techbutton").style.animation =
        "flash 2s step-start infinite";
    }
    research_points += 1;
    xp -= totalxp;
  ...
  military += Math.ceil(
    military * (modifiers.military / 10) * (1 + m.shield * 0.01)
  );
  for (const ef of temporaryeffects) {
    if (ef.type == "add") {
      food += ef.food;
  ...
  document.getElementById("xp_text").innerHTML =
    shorten(xp) + "/" + shorten(totalxp);
  ...
  document.getElementById("unemployed").innerHTML =
    "Unemployed People: " + shorten(unemployed);
  document.getElementById("resources").innerHTML = `Resources: ${shorten(
    resources
  )} (${resourcesgained >= 0 ? "+" : ""}${shorten(resourcesgained)})`;
}
